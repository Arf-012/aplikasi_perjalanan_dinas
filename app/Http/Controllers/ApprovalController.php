...
class ApprovalController extends Controller
{
    ...
    public function index(Request $request): View|JsonResponse
    {
        ...
        $highValueCount = (clone $baseQuery)->where('estimated_cost', '>=', 10000000)->count();
        ...
        if ($filter === 'flagged') {
            ...
            $query->where('estimated_cost', '>=', 10000000);
        }
    ...
    public function recordDecision(Request $request, string $id): RedirectResponse|JsonResponse
    {
        ...
        if ($decision === 'approve') {
            ...
            if (str_contains($trip->approval_status, 'Manager')) {
                $trip->approval_status = 'Finance Review';
                $message = "Travel Request #{$trip->id} authorized by Line Manager. Forwarded to Finance.";
            } elseif (str_contains($trip->approval_status, 'Finance')) {
                $trip->approval_status = 'Director SLA Review';
            ...
            $trip->approval_status = 'Clarification Requested';
    ...
    public function bulkApprove(Request $request): RedirectResponse|JsonResponse
    {
        ...
        $updatedCount = TravelRequest::whereIn('id', $validated['trip_ids'])
            ...
            ->update(['approval_status' => 'Finance Review']);
